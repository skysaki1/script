...
local X23 = Fluent:CreateWindow({
    Title = "Toto hub 3.0.4" .. Fluent.Version,
    SubTitle = "By Toto hub",
    TabWidth = 160,
...
local Tabs = {
    Main = X23:AddTab({ Title = "Main", Icon = "6026568198" }),
    Settings = X23:AddTab({ Title = "Settings", Icon = "settings" })
}

local N = {
    Ss = X23:AddTab({ Title = "Stats", Icon = "7040410130" })
}


local L = {
    P = X23:AddTab({ Title = "Combat", Icon = "7251993295" })
}

local G = {
    T = X23:AddTab({ Title = "Teleport", Icon = "6035190846" })
}

local A = {
    R = X23:AddTab({ Title = "Dungeon", Icon = "7044284832" })
}

local Y = {
    D = X23:AddTab({ Title = "Fruit", Icon = "7044233235" })
}

local k = {
    S = X23:AddTab({ Title = "Shop", Icon = "6031265976" })
}

local Q = {
    Misc = X23:AddTab({ Title = "Misc", Icon = "6034509993" })
}

    Fluent:Notify({
        Title = "Toto hub 3.0.4",
        Content = "Added WhiteList",
        SubContent = "discord.gg/22KGQeUcA9", -- Optional
        Duration = 5 -- Set to nil to make the notification not disappear
    })

    Tabs.Main:AddParagraph({
        Title = "About Us",
        Content = "You can set fastattack and select weapon at setting"
    })

local Fat = Tabs.Settings:AddToggle("Enable Fast Attack", {Title = "Fast attack", Default = false })

    Fat:OnChanged(function()
        local SuperFastMode = Options.Fast.Value -- เปลี่ยนเป็นจริงถ้าคุณต้องการโจมตี Super Super Super Fast (เช่นการฆ่าทันที) แต่จะทำให้เกมเตะคุณมากกว่าโหมดปกติ
...
local BringMob = Tabs.Settings:AddToggle("Enable Bring Mob", {Title = "Bring Mob", Default = false })

    BringMob:OnChanged(function()
        _G.BringMonster = Options.Bring.Value
    end)

    Options.Bring:SetValue(false)
    ...
    local Haki = Tabs.Settings:AddToggle("Auto Open Buso Haki", {Title = "Buso Haki", Default = true })

    Haki:OnChanged(function()
        _G.AUTOHAKI = Options.Haki.Value
    end)

    Options.Haki:SetValue(true)
...
local click = Tabs.Settings:AddToggle("Auto Click", {Title = "Auto Click", Default = false })

    click:OnChanged(function()
        _G.AutoClick = Options.Click.Value
    end)

    Options.Click:SetValue(false)
    ...
    local White = Tabs.Settings:AddToggle("White Screen", {Title = "White Screen", Default = false })

    White:OnChanged(function()
        G.WhiteScreen = Options.Wh.Value
    ...
    Options.Wh:SetValue(false)

    local Btpp = Tabs.Settings:AddToggle("Bypass TP", {Title = "Bypass Tp", Default = true })

    Btpp:OnChanged(function()
        BypassTP = Options.Btppp.Value
    end)

    Options.Btppp:SetValue(true)

    local DD = Tabs.Settings:AddDropdown("Select Weapon", {
        Title = "Select Weapon",
    ...
    task.spawn(function()
        ...
        AutoFarm:OnChanged(function()
            _G.AutoFarm = Options.Farm.Value
            StopTween(_G.AutoFarm)
        end)
    
        Options.Farm:SetValue(false)
        ...
        local BossName = Tabs.Main:AddDropdown("Select Boss", {
            Title = "Why you so cute",
            Values = Boss,
            Multi = false,
            Default = 1,
        })
        ...
        Tabs.Main:AddButton({
            Title = "refresh boss",
            Description = "thank you for using toto hub",
            Callback = function()
                BossName:Clear()
                for i, v in pairs(game:GetService("ReplicatedStorage"):GetChildren()) do
                if string.find(v.Name, "Boss") then
        ...
        local FarmBoss = Tabs.Main:AddToggle("AUto Farm Selected Boss", {Title = "Powered by Mama hub Maruko", Default = false })

    Toggle:OnChanged(function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
                _G.AutoFarmBoss = Options.FarmBoss.Value
                StopTween(_G.AutoFarmBoss)
    end)

    Options.FarmBoss:SetValue(false)
    ...
    local AllBoss = Tabs.Main:AddToggle("Auto Farm All Boss", {Title = "Powered By Uranium hub 2.0", Default = false })

    Toggle:OnChanged(function()
        _G.AutoAllBoss = Options.AllBos.Value
        StopTween(_G.AutoAllBoss)
    end)

    Options.AllBos:SetValue(false)
